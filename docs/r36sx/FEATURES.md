...
- Local-file playback uses the validated pre-tuning H.OS profile from commit
  `ac065e1`: audio-master synchronization, vendor-default quick-mode timing,
  and the original controls/subtitle overlay schedule. Experimental packet
  buffering, custom late-frame thresholds, RAM-only logging, and OSD blanking
  were reverted together after they caused jumpy motion or a startup freeze.
