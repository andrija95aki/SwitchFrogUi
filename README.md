...
| **Media and documents** | Rockbox Music with card-root browsing, a Winamp-style VFD skin and FFT visualizer; stock-paced hardware-decoded Videos with low overlay overhead; SRT/WebVTT subtitles and timing offsets; an internal Photo viewer with folder navigation, rotate, 12× zoom and pan; Ebooks; public-domain samples; whole-card Files with remembered folder cursors, automatic handlers plus a dimmed popup, multi-select, and confirmed recursive copy/cut/paste/delete, rename, and folder creation. |
